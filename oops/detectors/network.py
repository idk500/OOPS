...
class NetworkConnectivityDetector(DetectionRule):
        ...
        for type_key, stats in type_stats.items():
            ...
            if stats['failed'] > 0:
                if stats['success'] > 0:
                    ...
                    # 全部失败
                    if is_critical:
                        message_parts.append(f"{type_name}: ❌ 全部失败 ({stats['total']}个)")
                    else:
                        message_parts.append(f"{type_name}: ⚠️ 全部失败 ({stats['total']}个，可选)")
    ...
    async def _check_mirror_site(self, mirror_url: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(mirror_url, timeout=aiohttp.ClientTimeout(total=self.timeout)) as response:
                    response_time = (time.time() - start_time) * 1000
    ...
    async def _check_website(self, website_url: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(website_url, timeout=aiohttp.ClientTimeout(total=self.timeout)) as response:
                    response_time = (time.time() - start_time) * 1000
    ...
    async def _check_mihoyo_api(self, api_url: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=self.timeout)) as response:
                    response_time = (time.time() - start_time) * 1000
    ...
    async def _check_github_proxy(self, proxy_url: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(proxy_url, timeout=aiohttp.ClientTimeout(total=self.timeout)) as response:
                    response_time = (time.time() - start_time) * 1000
    ...
    def _analyze_network_status(self, results: Dict[str, Any]) -> str:
        ...
        # 检查可选组（全部失败才警告）
        optional_groups = ['github_proxy', 'mihoyo_api', 'mirror_site', 'project_website']
        has_warning = False
        ...
        for group_name in optional_groups:
            ...
            if group_results:
                # 检查是否全部失败
                all_failed = all(r.get('status') in ['failure', 'timeout', 'error'] for r in group_results)
                if all_failed:
                    has_warning = True
                    break
    ...
    def get_fix_suggestion(self, result: Dict[str, Any]) -> str:
        ...
        # Git仓库建议
        git_failed = [url for url, d in groups['git_repo'] if d.get('status') != 'success']
        git_success = [url for url, d in groups['git_repo'] if d.get('status') == 'success']
        ...
        # PyPI源建议
        pypi_failed = [url for url, d in groups['pypi_source'] if d.get('status') != 'success']
        pypi_success = [url for url, d in groups['pypi_source'] if d.get('status') == 'success']
        ...
        # GitHub代理建议
        proxy_failed = [url for url, d in groups['github_proxy'] if d.get('status') != 'success']
        proxy_success = [url for url, d in groups['github_proxy'] if d.get('status') == 'success']
