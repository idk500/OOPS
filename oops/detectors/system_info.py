...
class SystemInfoDetector:
    ...
    def _validate_hardware(self, system_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if disk_type != "Unknown":
            validation["disk_type"] = self.hardware_validator.validate_disk_type(
                disk_type
            )
    ...
    def _validate_display_settings(
    ...
    def get_fix_suggestion(self, result: Dict[str, Any]) -> str:
        ...
        # 显示设置建议
        if "display_settings" in validation and validation["display_settings"].get(
            "warning"
        ):
