...
class IssueMatcher:
    ...
    def search_issues(self, query: str) -> List[Dict[str, Any]]:
        ...
        # 搜索安装问题
        for keyword, issue_info in self.installation_issues.items():
            if query_lower in keyword.lower() or query_lower in issue_info['description'].lower():
                results.append({
                    'type': 'installation',
                    'keyword': keyword,
                    'description': issue_info['description'],
...
class SolutionRecommender:
    ...
    def recommend_solutions(self, diagnostic_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """基于诊断结果推荐解决方案
