...
class ProjectDetector:
    ...
    def generate_config_from_detection(self, detection_result: Dict[str, Any]) -> Dict[str, Any]:
        """根据检测结果生成配置
    ...
    def list_potential_projects(self, search_paths: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """列出可能的项目目录
...
def auto_detect_and_configure() -> Optional[Dict[str, Any]]:
    ...
    if detection:
        logger.info(f"✅ 在父目录检测到项目: {detection['project_name']} (深度: {detection['depth']})")
        return detector.generate_config_from_detection(detection)
