...
class ReportGenerator:
    ...
    def _generate_html_report(self, results: List[CheckResult], 
                            project_name: str, 
                            summary: Dict[str, Any]) -> str:
        """生成HTML报告"""
        # 构建报告内容
    ...
    def _generate_json_report(self, results: List[CheckResult], 
                            project_name: str, 
                            summary: Dict[str, Any]) -> str:
        """生成JSON报告"""
        report_data = {
    ...
    def _generate_markdown_report(self, results: List[CheckResult], 
                                project_name: str, 
                                summary: Dict[str, Any]) -> str:
        """生成Markdown报告"""
        content_parts = []
        ...
        if self.config.include_details:
            ...
            for result in results:
                ...
                content_parts.append(f"### {severity_emoji} {result.check_name} {status_emoji}")
                content_parts.append(f"- **状态**: {result.status.value}")
...
<body>
    ...
    def _get_html_critical_issues_section(self, critical_results: List[CheckResult]) -> str:
        """获取HTML关键问题部分"""
        content = """
        <div class="section">
    ...
    def _get_html_detailed_results_section(self, results: List[CheckResult]) -> str:
        ...
        sorted_results = sorted(results, key=lambda r: severity_order.get(r.severity, 4))
        ...
        if result.details:
            ...
            if failed_items or warning_items:
                ...
                if success_items and len(success_items) <= 5:  # 只显示少量成功项
                    details_html += "<div class='success-items'><strong>✅ 通过项:</strong><ul>"
    ...
    def _extract_fix_suggestions(self, results: List[CheckResult]) -> Dict[str, List[str]]:
        """提取修复建议并按类别分组"""
        suggestions = {
            "网络问题": [],
            "环境依赖": [],
        ...
        for result in results:
            if result.fix_suggestion and result.severity in [SeverityLevel.CRITICAL, SeverityLevel.ERROR, SeverityLevel.WARNING]:
                # 简单的关键词分类
                suggestion = result.fix_suggestion
                check_name = result.check_name.lower()
                
                if any(keyword in check_name for keyword in ['network', 'git', 'pypi', 'mirror']):
                    suggestions["网络问题"].append(suggestion)
                elif any(keyword in check_name for keyword in ['environment', 'python', 'dependency', 'library']):
                    suggestions["环境依赖"].append(suggestion)
                elif any(keyword in check_name for keyword in ['path', 'directory', 'permission']):
                    suggestions["路径规范"].append(suggestion)
                elif any(keyword in check_name for keyword in ['system', 'config', 'setting']):
