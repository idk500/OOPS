...
class DiagnosticSuite:
    ...
    async def run_diagnostics(self, project_name: str, 
                            selected_checks: Optional[List[str]] = None) -> List[CheckResult]:
        """运行诊断检测"""
        ...
        for check_name in checks_to_run:
            if check_name in self.detection_rules:
                task = self._run_single_check(
                    check_name,
                    self.detection_rules[check_name],
                    project_config
                )
                tasks.append(task)
    ...
    def _show_detection_plan(self, checks_to_run: List[str], project_config: Dict[str, Any]):
        """显示检测计划 - 简化显示"""
        ...
        if 'path_validation' in checks_to_run:
            ...
            install_path = project_config.get('project', {}).get('paths', {}).get('install_path', '未设置')
            print(f"   +- 路径规范 (安装路径: {install_path})")
        
        print("   +- 检测准备完成")
        print()
    
    def _determine_checks_to_run(self, project_config: Dict[str, Any],
                               selected_checks: Optional[List[str]]) -> List[str]:
        """确定需要运行的检测项目"""
        if selected_checks:
    ...
    async def _run_single_check(self, check_name: str, rule: DetectionRule, 
    ...
    def _create_hardware_compatibility_result(self, system_info_result: Dict[str, Any], start_time: datetime):
        """从系统信息中创建硬件适配检测结果"""
        ...
        if 'display_settings' in validation:
            ...
            if not display_val.get('valid'):
                ...
                warnings.extend([f"显示设置: {warning}" for warning in display_warnings])
                has_warning = True
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        error_issues = len([r for r in self.results if r.severity == SeverityLevel.ERROR])
        warning_issues = len([r for r in self.results if r.severity == SeverityLevel.WARNING])
