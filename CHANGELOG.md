...
### 🖥️ GUI 引导窗(替代黑控制台)
- 双击 oops.exe → 弹出**置顶 tkinter 引导窗**:进度、倒数、错误都在窗口里显示。
- 双击时**隐藏控制台**(不再闪黑框);出错时窗口保留,方便手机拍照反馈。
- boot 输出走 `reporter`(GUI 传窗口方法、控制台用 print);恢复**可见倒数**(无人值守,到点自动继续)。

### 🌐 GBK 编码修复
- 所有 git 子进程输出用 UTF-8 解码(`run_git` / `git_provider`),不再因中文/emoji 触发 GBK 报错。

### 📦 其他
- 新增 `oops/actions/boot_gui.py`;控制台隐藏仅作用于"打包 exe + 双击(无参数)"场景,CLI 子命令不受影响。
- 测试 26 通过。
...
1. **持续集成流程优化**
   - ✅ 统一本地和CI环境的flake8检查规则
   - ✅ 调整最大行长度限制从88到127字符，与CI保持一致
   - ✅ 移除冗余的本地.flake8配置文件
   - ✅ 创建与CI流程一致的新配置文件
...
1. **flake8配置一致性问题**
   - ❌ 之前：本地检查规则与CI流程不一致，导致检查结果差异
   - ✅ 现在：统一配置，确保开发和CI环境的一致性
